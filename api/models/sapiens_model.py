...
from sapiens_util import udp_decode, top_down_affine_transform
...
class SapiensProcessor:
    """Wrapper for Sapiens pose estimation models"""
    def __init__(self, checkpoint, device="cuda:0", batch_size=8, window_size=5, 
                 heatmap_scale=4, shape=(1024, 768), output_folder="static/results", 
                 save_img_flag=True):
    ...
    def _apply_butterworth_filter(self, keypoints_batch):
        """Apply Butterworth filter to keypoints using a sliding window approach"""
        filtered_batch = []
    ...
    def process_video(self, input_path, output_path, method="original", filter_window=5, 
                      output_format="mp4", kpt_thr=0.3, radius=6, thickness=3,
                      save_keypoints=False, keypoints_path=None, sample_rate=1):
        """
        ...
        Args:
            ...
            method: Filtering method ('original' or 'butterworth')
            ...
            keypoints_path: Path to save keypoint data (if save_keypoints is True)
            sample_rate: Save every Nth frame's keypoints (when save_keypoints is True)
        ...
        try:
            ...
            for batch_idx, (batch_image_name, batch_orig_imgs, batch_imgs) in enumerate(inference_dataloader):
                ...
                # For keypoint extraction (if enabled)
                if save_keypoints and keypoints_path:
                    # Only save frames at the specified sample rate
                        ...
                        if frame_num % sample_rate == 0:
                            ...
                            for heatmap, centre, scale in zip(
                                ...
                                kpts, scores = udp_decode(
                                    ...
                                    input_shape[1:],
                                    (int(input_shape[1] / scale), int(input_shape[2] / scale)),
                                )
            ...
            if save_keypoints and keypoints_path and all_coco_keypoints:
                print(f"Saving {len(all_coco_keypoints)} keypoints frames to {keypoints_path}")
                
                # Apply filtering to keypoints if method is not original
                if method != "original":
                    all_coco_keypoints = JointDataProcessor.apply_filter_to_keypoints(
                        all_coco_keypoints, method, filter_window
                    )
    ...
    def batch_inference_topdown(self, model: nn.Module, imgs: torch.Tensor, dtype=torch.bfloat16, flip=False):
